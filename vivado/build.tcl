...
set write_bitstream_prehook [file normalize [file join $script_dir hooks write_bitstream_prehook.tcl]]
...
if {$rx_refclk_p_pin ne "" || $tx_refclk_p_pin ne ""} {
  if {$rx_refclk_p_pin ne ""} {
    ...
    connect_bd_net $tx_refclk_p_pin [get_bd_ports sdi_refclk_p]
  }
} else {
...
if {$rx_refclk_n_pin ne "" || $tx_refclk_n_pin ne ""} {
  if {$rx_refclk_n_pin ne ""} {
    ...
    connect_bd_net $tx_refclk_n_pin [get_bd_ports sdi_refclk_n]
  }
} else {
...
wait_on_run synth_1
if {[file exists $write_bitstream_prehook]} {
  set_property STEPS.WRITE_BITSTREAM.TCL.PRE $write_bitstream_prehook [get_runs impl_1]
} else {
  puts "WARNING: ${write_bitstream_prehook} not found; write_bitstream DRC hook not installed"
}
